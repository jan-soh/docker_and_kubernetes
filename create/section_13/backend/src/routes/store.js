...
keyValueRouter.post('/', async (req, res) => {
    ...
    console.log(`${req.body} with ${key} and ${value}`);
    try {
        if (!key || !value) {
        ...
        await keyValue.save();
        return res.status(201).json({message: 'Key value pair created'});
    } catch (err) {
...
keyValueRouter.put('/:key', async (req, res) => {
    ...
    try {
        ...
        return res.status(200).json({message: 'Key value pair updated', key: keyValue.key, value: keyValue.value});
    } catch (err) {
        return res.status(500).json({message: 'Internal server error'});
    }
});
keyValueRouter.delete('/:key', async (req, res) => {
    ...
    try {
        ...
        return res.status(204).json({message: 'Key value pair deleted'});
    } catch (err) {
        return res.status(500).json({message: 'Internal server error'});
    }
});
